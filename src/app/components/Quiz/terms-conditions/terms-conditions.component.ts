...
//   updateActivedStatus(): void {
//     this.assessmentDetails.forEach((assessment) => {
//       const { assessmentId, userId, expiryDate, isLinkAccessed, isActive } = assessment;
//       const recordKey = `${assessmentId}_${userId}`;
  
//       // Check if the link has expired and never been accessed
//       const shouldDeactivate = this.isLinkExpired(expiryDate) && !isLinkAccessed;
  
//       if (shouldDeactivate && isActive !== false) {
//         console.log(`Updating status for ${recordKey}: Link expired and never accessed.`);
//         this.updateState(recordKey, { isActive: false }).catch((error) => {
...
//     const { assessmentId, userId, expiryDate, isLinkAccessed, isActive } =
//       assessment;
//     const recordKey = `${assessmentId}_${userId}`;
